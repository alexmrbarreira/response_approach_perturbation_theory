...
who developed the response approach and used it, as a first application, to address the problem of accurate analytical calculations of covariance matrices. This code includes the Gaussian (G) term and the connected non-Gaussian (cNG) term up to 1-loop. For the remaining super-sample covariance (SSC) term see [this other code](https://github.com/alexmrbarreira/gravitational_lensing_SSC).
